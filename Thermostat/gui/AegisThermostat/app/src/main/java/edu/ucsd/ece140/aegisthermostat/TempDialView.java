package edu.ucsd.ece140.aegisthermostat;
...
public class TempDialView extends View {
    ...
    double max = 100;
    float size = 1000;
    RectF oval = new RectF((float) (size*0.10), (float) (size*0.10), (float) (size*0.84), (float) (size*0.84));
    ...
    double findDegree(double displayValue){
        ...
        if(displayValue>90) displayValue=85;
    ...
    void drawMark(Canvas canvas, double value, int aColor, String text){
        ...
        for (Rect existingBounds : textBounds) {
            if (Rect.intersects(bounds, existingBounds)) {
                collides = true;
                break;
            }
        }
